...
- [ ] 1. Fix DB SSL env-key drift and config-source consistency
  ...
  **Acceptance Criteria**:
  - [ ] `grep -R 'DB_SSLMODE' backend pkg . -n` returns no stale tracked usage.
  - [ ] `grep -R 'DB_SSL_MODE' backend/.env-example backend/pkg/config/config.go -n` returns aligned usage.
  - [ ] `cd backend && go build ./...` exits `0`.

- [ ] 2. Add backward-compatible OAuth token read handling for legacy plaintext rows
  ...
  **Acceptance Criteria**:
  - [ ] `cd backend && go test ./internal/repository/postgres -count=1 -run 'TestAuthRepository_' -v` exits `0` if repo tests are added; otherwise equivalent focused auth repository/service coverage passes.
  - [ ] `grep -n 'crypto.Decrypt' backend/internal/repository/postgres/auth.go` still shows decrypt-on-read behavior, but with guarded compatibility for legacy plaintext rows.
  - [ ] `cd backend && go build ./...` exits `0`.

- [ ] 3. Clean up auth-service error semantics around already-domain-aware repositories
  ...
  **Acceptance Criteria**:
  - [ ] `grep -n 'errors.New("user not found"' backend/internal/service/auth_service.go` returns no stale ad hoc not-found mapping if replaced by a stronger domain-aware path.
  - [ ] `cd backend && go test ./internal/service -count=1 -run 'TestGetMe_|TestRefreshToken_|TestCompleteAuth_' -v` exits `0`.
  - [ ] `cd backend && go build ./...` exits `0`.

- [ ] 4. Sync docs and env guidance with current backend security/infra reality
  ...
  **Acceptance Criteria**:
  - [ ] `grep -n 'Redpanda' AGENTS.md backend/AGENTS.md` returns current-state wording, not future-state wording.
  - [ ] `grep -n 'caarlos0/env\|encrypted' backend/pkg/AGENTS.md AGENTS.md` returns current-state wording.
  - [ ] `grep -n 'RBAC/Casbin' AGENTS.md` does not regress current implemented/planned status while editing adjacent docs.
  - [ ] `grep -R 'DB_SSL_MODE\|OAUTH_TOKEN_ENCRYPTION_KEY' backend/.env-example backend/pkg/config/config.go -n` returns current aligned guidance.

- [ ] 5. Add focused verification and residual regression checks for the already-landed fixes
  ...
  **Acceptance Criteria**:
  - [ ] `cd backend && go build ./...` exits `0`.
  - [ ] `cd backend && go test ./internal/service -count=1 -run 'TestGetMe_|TestRefreshToken_|TestCompleteAuth_' -v` exits `0`.
  - [ ] `cd backend && go test ./internal/repository/postgres -count=1 -run 'TestAuthRepository_' -v` exits `0` if repository auth tests are added.
  - [ ] `grep -n 'redpanda:' docker-compose.yml` returns current broker config.
  - [ ] `grep -n 'Price    int64' backend/internal/domain/entity/listing.go` returns the integer-money invariant.
