...
export default async function PublicListingDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const beds = specs.bedrooms || dummy.beds;
  const baths = specs.bathrooms || dummy.baths;
  const sqft = specs.building_area_sqm
    ? Math.round(specs.building_area_sqm * 10.7639)
    : dummy.sqft;
        ...
        <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col justify-between gap-6 pb-6 md:flex-row md:items-start">
            ...
            <div className="flex flex-col items-end justify-between gap-10">
              <button aria-label="More options" className="hidden h-10 w-10 items-center justify-center rounded-full border border-border bg-white text-foreground shadow-sm transition-colors hover:bg-slate-50 md:flex">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 17h2"/><path d="M11 12h2"/><path d="M11 7h2"/></svg>
              </button>

              <div className="flex flex-row items-center gap-2">
                <button className="inline-flex h-10 items-center justify-center rounded-full bg-slate-900 px-6 text-sm font-semibold text-white shadow transition-colors hover:bg-slate-900/90">
                  Schedule a tour
                </button>
                <button aria-label="Save" className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-border bg-white text-foreground shadow-sm transition-colors hover:bg-slate-50">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg>
                </button>
                <button aria-label="Share" className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-border bg-white text-foreground shadow-sm transition-colors hover:bg-slate-50">
          ...
          <section className="grid gap-8 lg:grid-cols-[250px_1fr]">
            ...
            <div>
              <div className="mb-6 flex flex-wrap gap-2">
                <span className="rounded-full border border-border px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-foreground shadow-sm">House</span>
                <span className="rounded-full border border-border px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-foreground shadow-sm">Cooling</span>
                <span className="rounded-full border border-border px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-foreground shadow-sm">Heating</span>
                <span className="rounded-full border border-border px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-foreground shadow-sm">Year built 1920</span>
                <span className="rounded-full border border-border px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-foreground shadow-sm">0.19 Acres</span>
              </div>
          ...
          <section className="mt-16 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
               <div className="relative aspect-4/3 w-full overflow-hidden bg-muted">
                 <Image alt="gallery 2" fill src={images[2]} className="object-cover" unoptimized/>
               </div>
               <div className="grid grid-rows-2 gap-4">
                 <div className="relative w-full h-full overflow-hidden bg-muted">
                    <Image alt="gallery 3" fill src={images[3]} className="object-cover" unoptimized/>
                 </div>
                 <div className="relative w-full h-full overflow-hidden bg-muted">
                    <Image alt="gallery 4" fill src={images[4]} className="object-cover" unoptimized/>
                 </div>
               </div>
            </div>
            <div className="relative aspect-16/6 w-full overflow-hidden bg-muted">
               <Image alt="gallery 5" fill src={images[5]} className="object-cover" unoptimized/>
            </div>
          </section>
          ...
          <section className="mt-16 flex flex-col">
             <div className="grid grid-cols-2 gap-4 h-[240px] w-full overflow-hidden md:h-[400px]">
                <div className="relative w-full h-full overflow-hidden bg-muted">
                   <Image alt="gallery left" fill src={images[0]} className="object-cover" unoptimized/>
                </div>
                <div className="relative w-full h-full overflow-hidden bg-muted">
                   <Image alt="gallery right" fill src={images[5]} className="object-cover" unoptimized/>
                </div>
             </div>
             <div className="mt-6 flex items-center justify-between">
               ...
               <div className="flex gap-2">
                 <button className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-white shadow hover:opacity-90">
                    &lt;
                 </button>
                 <button className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-white shadow hover:opacity-90">
                    &gt;
