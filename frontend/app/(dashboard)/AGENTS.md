...
Admin-only dashboard routes. Server-first auth gating, dashboard workspace layouts, and listing-management page entrypoints live here.

## WHERE TO LOOK

| Task | Location | Notes |
|------|----------|-------|
| Shared protected shell | `layout.tsx` | must gate with `requireUser({ intent: "seller" })` |
| Dashboard overview | `dashboard/page.tsx` | server-rendered metrics + client refresh island |
...
- Redirect unauthenticated users to `/seller/login`, not `/`.
- Pull auth/session checks from `features/auth/server`, not ad hoc route code.
- Keep admin-vs-user gating in auth helpers/layouts, not in leaf page JSX.
