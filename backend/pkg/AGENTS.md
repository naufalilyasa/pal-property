...
- Search indexing config now lives in the same contract with `ELASTIC_ADDRESS`, optional `ELASTIC_USERNAME` / `ELASTIC_PASSWORD`, and `ELASTIC_INDEX_LISTINGS` for the modular-monolith search index.
