...
func setupAuthzTestState(db *gorm.DB) error {
	statements := []string{
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS uq_casbin_rule_tuple ON casbin_rule (ptype, v0, v1, v2, v3, v4, v5)`,
		`CREATE INDEX IF NOT EXISTS idx_casbin_rule_lookup ON casbin_rule (ptype, v0, v1, v2)`,
...
func newAuthzService(db *gorm.DB) (*authz.Service, error) {
	if err := setupAuthzTestState(db); err != nil {
		return nil, err
	}

	return authz.NewService(db)
}
