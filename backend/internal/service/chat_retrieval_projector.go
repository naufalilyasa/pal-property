...
func chatRetrievalDocumentFromEvent(event domain.ListingEvent) chatRetrievalSearchDocument {
	return chatRetrievalSearchDocument{
...
func buildChatEmbeddingInput(document chatRetrievalSearchDocument) string {
	parts := []string{document.Title, document.Slug, document.TransactionType, document.Currency}
	if document.DescriptionExcerpt != nil {
		parts = append(parts, *document.DescriptionExcerpt)
	}
	if document.LocationVillage != nil {
		parts = append(parts, *document.LocationVillage)
	}
	if document.LocationDistrict != nil {
		parts = append(parts, *document.LocationDistrict)
	}
	if document.LocationCity != nil {
		parts = append(parts, *document.LocationCity)
	}
	if document.LocationProvince != nil {
		parts = append(parts, *document.LocationProvince)
	}
	if document.Category != nil {
		parts = append(parts, document.Category.Name, document.Category.Slug)
	}
